...
import com.wisenable.dms.datasource.entities.SystemConfigExample;
import com.wisenable.dms.datasource.entities.User;
import com.wisenable.dms.datasource.mappers.ShipperMapper;
import com.wisenable.dms.datasource.mappers.SystemConfigMapper;
import com.wisenable.dms.datasource.mappers.SystemConfigMapperEx;
...
import com.wisenable.dms.utils.FileUtils;
...
public class SystemConfigService {
    ...
    public List<SystemConfig> select(String companyName, int offset, int rows)throws Exception {
        ...
        try {
            ...
            List<Shipper> shippers = shipperMapper.selectByTenantId();
            if (Tools.isNotEmpty(shippers) && Tools.isNotEmpty(list) ) {
                SystemConfig config = list.get(0);
                config.setAppId(shippers.get(0).getAppId());
                config.setKey(shippers.get(0).getSecretKey());
            }
        } catch(Exception e){
            DMSException.readFail(logger, e);
        }
