...
import com.wisenable.dms.datasource.vo.TreeNodeEx;
import com.wisenable.dms.datasource.vo.UserBindWistoreInfo;
import com.wisenable.dms.datasource.vo.WistoreMsgDTO;
import com.wisenable.dms.exception.BusinessParamCheckingException;
import com.wisenable.dms.exception.BusinessRunTimeException;
import com.wisenable.dms.service.redis.RedisService;
import com.wisenable.dms.service.role.RoleService;
import com.wisenable.dms.service.tenant.TenantService;
...
import com.wisenable.dms.utils.RandImageUtil;
import com.wisenable.dms.utils.Tools;
import com.wisenable.dms.utils.*;
...
public class UserController {
    ...
    public BaseResponseInfo getUserBtnByCurrentUser(HttpServletRequest request) {
        ...
        try {
            ...
            if(!"admin".equals(loginName)) {
                ...
                JSONObject btnRoleObj = new JSONObject();
                btnRoleObj.put("url", "/system/role");
                btnRoleObj.put("btnStr", "1,2,3,5,7");
                btnArr.add(btnRoleObj);

                JSONObject btnUserObj = new JSONObject();
                btnUserObj.put("url", "/system/user");
                btnUserObj.put("btnStr", "1");
                btnArr.add(btnUserObj);
                data.put("userBtn", btnArr);
            }
            res.code = 200;
            res.data = data;
        } catch(Exception e){
    ...
    @GetMapping(value = "/wechat/bindingInfo")
    @ApiOperation(value = "员工绑定微信公众号")
    public BaseResponseObject<UserBindWistoreInfo> bindingInfo(@RequestParam("userId") Long userId, HttpServletRequest request) throws IOException {
        if (userId == null) {
            return BaseResponseObject.error(201, "userId不能为空");
        }
        //
        User user = userService.getUser(userId);
        if (user == null) {
            return BaseResponseObject.error(200, "用户不存在");
        }

        WistoreMsgDTO wistoreMsgDTO = new WistoreMsgDTO();
        wistoreMsgDTO.setPlatform("DMS");
        wistoreMsgDTO.setEmployeeId(userId);
        wistoreMsgDTO.setCompanyId(user.getTenantId());

        UserBindWistoreInfo userBindWistoreInfo = new UserBindWistoreInfo();
        userBindWistoreInfo.setUserId(userId);
        userBindWistoreInfo.setWistoreMsgOpenId(user.getWistoreMsgOpenId());
        userBindWistoreInfo.setBindQRCode(Tools.getQrCodeBase64(JSONUtil.toJsonStr(wistoreMsgDTO)));
        return BaseResponseObject.ok(userBindWistoreInfo);
    }

    @PostMapping(value = "/wechat/unbinding/{id}")
    @ApiOperation(value = "解绑微信公众号")
    public BaseResponseObject<Integer> unbinding(@PathVariable("id") Long userId, HttpServletRequest request) {
        if (userId == null) {
            return BaseResponseObject.error(201, "userId不能为空");
        }
        User user = new User();
        user.setId(userId);
        user.setWistoreMsgOpenId("");
        return BaseResponseObject.ok(userService.updateUserByObj(user));
    }
